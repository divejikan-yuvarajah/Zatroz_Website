...
| Project       | Admin `/admin/projects` (Mongo) | Create/edit drafts from real evidence only. Publish via A08 owner controls. Do not maintain a parallel live catalog in `src/content/projects.ts` after A10. Homepage featured order is `/admin/settings/featured`. |
...
| Feedback      | `src/content/feedback.ts`       | Only permitted testimonials or honest Zatroz project lessons. Never invent customer quotes or star ratings.                                                                                                        |
| Questions     | `src/content/home-questions.ts` | Homepage FAQ framing. Approve only with at least one approved FAQ or feedback item.                                                                                                                                |
| Final CTA     | `src/content/home-final-cta.ts` | Homepage closing invitation. Public render also needs a usable Contact/email/WhatsApp action — do not invent one.                                                                                                  |
...
| Business need | `src/content/business-needs.ts` | Keep four needs. Reference service IDs (not titles/paths). Set `primaryServiceId` within `serviceIds`. Approve explorer framing and needs together for the public strip.                                           |
| Process       | `src/content/process.ts`        | Keep four steps. Approve only when descriptions and customer outputs are honest and do not invent free forever support or fixed timelines.                                                                         |
| People        | `src/content/people.ts`         | Company intro and working principles. Do not invent founder cards — add real approved founders in `founders.ts` only.                                                                                              |
