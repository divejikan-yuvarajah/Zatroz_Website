...
| `SUPABASE_URL`                   | Server config (not itself a secret) | Enquiry persistence (later)                      |
| `SUPABASE_SECRET_KEY`            | Server secret                       | Enquiry persistence (later)                      |
...
`SUPABASE_SECRET_KEY` is our planned **application** name for the supported **server** credential from Supabase setup. Never put a publishable/anon key there. No public Supabase browser client is required for this marketing-site enquiry design.
...
| Item                                                  | Status              |
| ----------------------------------------------------- | ------------------- |
| `.env.example` with blank future values               | Done                |
...
| Eager Supabase / Resend / Turnstile / cron clients    | **Not** implemented |
