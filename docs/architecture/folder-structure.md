...
| `src/lib/`                                  | Shared utilities only; **no** secret barrel exports      | Includes `mongodb/`, `email/` (pure adapter/templates)     |
| `src/server/`                               | Server-only env access and content selectors             | `env.ts`, `mongodb.ts`, `email/`, public project selectors |
| `docs/setup/transactional-email.md`         | Resend domain/API/recipient runbook                      | Step 50                                                    |
