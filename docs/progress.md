...
- Database privilege matrix, Server Action request policy, MongoDB rate-limit helper, safe errors/logs, readiness + recovery runbooks.
- Narrow enquiry repository + always-deny auth gates. No live enquiry endpoint; Contact form stays off. Next work is A01–A12 (not Step 47).
...
### Admin A03 (2026-09-19)

- Branch: `feature/a03-admin-shell` from `feature/a02-admin-auth`.
- Protected `/admin` console (no marketing chrome), role-filtered nav, owner `/admin/staff` role updates, Mongo dashboard counts + recent audit list.
...
### Admin A06 (2026-09-19)

- Branch: `feature/a06-admin-case-study` from `feature/a05-admin-projects`.
- `/admin/projects/[id]/story` structured case-study editor: paragraph/list blocks, features/technologies/outcomes, ordered gallery, optional testimonial, internal review notes.
...
### Admin A10 (2026-09-20)

- Branch: `feature/a10-repo-portfolio-migration` from `feature/a09-featured-public-selectors`.
- One-time idempotent import: `npm run migrate:repo-portfolio -- --target <development|test|preview> [--apply]`.
- Drafts never auto-publish; approved summary/story pointers set independently; archived repo rows skipped; featured IDs only when public-ready.
- Repository `projectRecords` / `mediaRecords` remain empty and are retired as a parallel live source (Mongo admin is canonical).
- Docs: `docs/admin/step-a10.md`, updated `docs/work/admin-migration-inventory.md`.
- Checks: `npm run format` passed; `npm run check` passed (includes `test:admin-repo-migration`); `npm run build` passed (Next.js 16.3.5). Live Atlas apply **Not run** (0 approved repo rows → dry-run no-op). Next: **A11** (cache refresh / image cleanup). Step 47 remains blocked until A12.

### Admin A11 (2026-09-20)

- Branch: `feature/a11-cache-refresh-cleanup` from `feature/a10-repo-portfolio-migration`.
- Content jobs worker: idempotent enqueue/requeue, lease + backoff retries, CLI + `CRON_SECRET` HTTP route, admin `/admin/jobs`.
- Publish refresh prepares public Cloudinary derivatives; public catalog accepts durable `/` or `https://` storage hints.
...
- Branch: `feature/49-enquiry-spam-protection` from `feature/48-submission-protection`.
- Server-verified Cloudflare Turnstile after distributed rate limits and before enquiry insert; widget + Siteverify action `enquiry-submit`.
- Transport: `challenge-failed` result status; `turnstileToken` excluded from business fingerprint; fresh token on retry with retained attempt key.
- Readiness requires Turnstile site + secret; `APP_ENV=production` refuses Cloudflare test keys.
